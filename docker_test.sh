...
# sudo docker run -v "$(pwd)":/sim mvonbun/omnetpp:5.0 bash -c "cd simulations && ./run -u docker run -v "$(pwd)":/sim mvonbun/omnetpp:5.0 bash -c "cd simulations && ./run -u Cmdenv -c ApecConventional -r 1" &>/dev/null
